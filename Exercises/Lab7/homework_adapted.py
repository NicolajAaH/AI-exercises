...
def print_conditional_probability(network, conditionals_vars, conditionals_evidents):
    ...
    return network.get_conditional_probability(
            conditionals_vars,
            conditionals_evidents
        )
...
def car():
    # the values kept as dictionary SE KOMMENTARER I EXERCISE (BN) VÆRDIERNE I TABELLERNE ER FOR NÅR DET ER TRUE
    # (FALSE, TRUE)
    t1 = {(): (0.7, 0.3)} #DT
    t2 = {(): (0.8, 0.2)} #FTL
    t3 = {(): (0.7, 0.3)} #EM
    t4 = {('false',): (0.9, 0.1), ('true',): (0.3, 0.7)} #V
    t5 = {
        ('false', 'false'): (0.3, 0.7),
        ('true', 'false'): (0.4, 0.6),
        ('false', 'true'): (0.7, 0.3),
    ...
    damagedTire = Variable('DamagedTire', ('false', 'true'), t1)
    fuelTankLeaking = Variable('FuelTankLeaking', ('false', 'true'), t2)
    electronicsMalfunctioning = Variable('ElectronicsMalfunctioning', ('false', 'true'), t3)
    vibrations = Variable('Vibrations', ('false', 'true'), t4, [damagedTire])
    slowMaxSpeed = Variable('SlowMaxSpeed', ('false', 'true'), t5, [damagedTire, electronicsMalfunctioning])
    highConsumption = Variable('HighConsumption', ('false', 'true'), t6, [damagedTire, fuelTankLeaking, electronicsMalfunctioning])

    variables = [damagedTire, fuelTankLeaking, electronicsMalfunctioning, vibrations, slowMaxSpeed, highConsumption]
    ...
    conditionals_vars = {'DamagedTire': 'true'}
    conditionals_evidents = {'Vibrations': 'true', 'SlowMaxSpeed': 'true', 'HighConsumption': 'false'}
    print('Damaged tire?')
    damag = print_conditional_probability(network, conditionals_vars, conditionals_evidents) # SE KOMMENTARER I BN

    print('')

    conditionals_vars = {'ElectronicsMalfunctioning': 'true'}
    conditionals_evidents = {'Vibrations': 'true', 'SlowMaxSpeed': 'true', 'HighConsumption': 'false'}
    print('Electronics Malfunctioning?:')
    elec = print_conditional_probability(network, conditionals_vars, conditionals_evidents)

    print('')

    conditionals_vars = {'FuelTankLeaking': 'true'}
    conditionals_evidents = {'Vibrations': 'true', 'SlowMaxSpeed': 'true', 'HighConsumption': 'false'}
    print('Fuel Tank Leaking?: ')
    fuel = print_conditional_probability(network, conditionals_vars, conditionals_evidents)
    ...
    temp = max(damag, elec, fuel)
    print('What is most likely wrong?:')
    if temp == damag:
        print("Damaged tire: " + str(damag))
    elif temp == elec:
        print("Electronics Malfunctioning: " + str(elec))
    else:
        print("Fuel Tank Leaking: " + str(fuel))

car()

# Modify the Bayesian network program to calculate the probabilities for the network on the following slide.
# You are interested in finding out the cause of a malfunction of your car.
# You suspect a fault in your car, but you do not know in which component.
# You have noticed that there are vibrations while driving and it is difficult to reach the top speed.
# Your car does not consume more than usual.
