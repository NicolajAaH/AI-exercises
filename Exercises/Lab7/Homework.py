...
class Variable(object):
    ...
    def calculate_marginal_probability(self): # TODO VIRKER DEN?
        ...
        if self.ready:
            return
        sum = 0
        for x, y in self.probability_table.items():
            size = len(self.probability_table.items())
            sum = sum + y[1]/size
        self.marginal_probabilities = [1-sum, sum]
...
class BayesianNetwork(object):
    ...
    def get_joint_probability(self, values): # TODO MANGLER
        """ return the joint probability of the Nodes """
        val = 1
        for x in values.items():
            val = val * self.get_marginal_probability(self.get_variable(x[0]), x[1])
...
def print_conditional_probability(network, conditionals_vars, conditionals_evidents):
    ...
    temp = network.get_conditional_probability(
            conditionals_vars,
            conditionals_evidents
        )
    print("is {:f}".format(temp))
    return temp
...
def car():
    ...
    print('Damaged tire?')
    damag = print_conditional_probability(network, conditionals_vars, conditionals_evidents)
