...
class CSP:
    ...
    def recursive_backtracking(self, assignment): # implement
        if self.is_complete(assignment): # if assignment is complete then return assignment
            return assignment
        var = self.select_unassigned_variable(assignment)
        for value in self.order_domain_values(var, assignment):
            if self.is_consistent(var, value, assignment): # if value is consistent with assignment given constraints
                assignment[var] = value
                result = self.recursive_backtracking(assignment)
                if result is not None: # not equal failure
                    return result
                assignment[var] = None # remove value from assignment
        return None
